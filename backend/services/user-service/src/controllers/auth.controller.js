...
async function register(req, res) {
  try {
    ...
    // Si c'est une erreur de validation Joi, retourner les détails
    if (err.isJoi) {
...
async function login(req, res) {
  try {
    ...
    // Enhanced response with role information for frontend routing
    const response = {
      accessToken,
      refreshToken,
      user: {
      ...
      // Flag to indicate if user needs to choose interface (for admins)
